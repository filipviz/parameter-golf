...
def main() -> None:
    ...
    tok_params = [{"params": [base_model.tok_emb.weight], "lr": args.embed_lr, "base_lr": args.embed_lr}]
    if base_model.bigram is not None:
        tok_params.append({"params": [base_model.bigram.embed.weight], "lr": args.embed_lr, "base_lr": args.embed_lr})
    ...
    if args.warmup_steps > 0:
        ...
        for warmup_step in range(args.warmup_steps):
            ...
            for micro_step in range(grad_accum_steps):
                x, y = train_loader.next_batch(args.train_batch_tokens, args.seq_len, grad_accum_steps)
                with torch.autocast(device_type="cuda", dtype=torch.bfloat16, enabled=True):
    ...
    while True:
        ...
        grad_norm = torch.nn.utils.clip_grad_norm_(base_model.parameters(), args.grad_clip_norm) if args.grad_clip_norm > 0 else None
    ...
    eval_model = _build_model(args, device)
    ...
    if args.eval_stride > 0 and args.eval_stride < args.seq_len:
        torch.cuda.synchronize()
    ...
    if args.eval_stride != 64 and 64 < args.seq_len:
        torch.cuda.synchronize()
