...
# - 8 attention heads and 4x MLP expansion
# - vocab size 1024, sequence length 2048, tied embeddings
# - 786,432 train tokens per step with a ~10 minute cap
...
# Instead, we get approximately the same model (with a small hit) by quantizing the model to int8 and compressing with brotli.
...
class GPT(nn.Module):
    ...
    def _init_weights(self) -> None:
        residual_proj_std = self.init_std / math.sqrt(2.0 * self.num_layers)
        for module in self.modules():
            if isinstance(module, nn.Embedding):
                nn.init.normal_(module.weight, mean=0.0, std=self.init_std)
            elif isinstance(module, nn.Linear):
                std = residual_proj_std if getattr(module, "_residual_proj", False) else self.init_std
                nn.init.normal_(module.weight, mean=0.0, std=std)
                if module.bias is not None:
                    nn.init.zeros_(module.bias)

    def forward(self, input_ids: Tensor, target_ids: Tensor) -> Tensor:
        x = norm(self.tok_emb(input_ids))
        for block in self.blocks:
            x = block(x)
        x = norm(x).reshape(-1, x.size(-1))
...
def main() -> None:
    ...
    # Optimizer split:
    # - tied token embedding/head (Adam) uses EMBED_LR
    ...
    log0(
        f"model_shape vocab_size:{args.vocab_size} layers:{args.num_layers} dim:{args.model_dim} "
        f"heads:{args.num_heads} mlp_mult:{args.mlp_mult} init_std:{args.init_std}"
    )
    log0(f"attention_mode:mha num_heads:{args.num_heads}")
    log0(
        f"tie_embeddings:True embed_lr:{args.embed_lr} matrix_lr:{args.matrix_lr} "
    ...
    # the compressed int8+brotli artifact and validate the round-tripped weights.
    ...
    if master_process:
        with open("final_model.int8.pt.br", "wb") as f:
            f.write(quant_blob)
        quant_file_bytes = os.path.getsize("final_model.int8.pt.br")
        code_bytes = len(code.encode("utf-8"))
    ...
    if distributed:
        dist.barrier()
    with open("final_model.int8.pt.br", "rb") as f:
