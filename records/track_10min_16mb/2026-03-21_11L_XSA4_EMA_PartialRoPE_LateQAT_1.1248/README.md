...
| Late QAT | None | STE int6 at lr_scale < 0.1 |
...
3. **Late QAT**. STE (Straight-Through Estimator) int6 fake-quantization activated only when the learning rate schedule drops below 10% (lr_scale < 0.1). This occurs ~300 steps before wallclock cap. Cuts int6 quantization degradation by 3x (0.0067 vs ~0.020) with zero cost during 96% of training. Inspired by arXiv:2505.14302.
